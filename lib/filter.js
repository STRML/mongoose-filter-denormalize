...
* retrieved, to ensure that JSON send to a user does not include inappropriate fields.
...
module.exports = function filter(schema, options) {
    ...
    schema.statics._applyFilter = function _applyFilter(input, filters){
        ...
        _.each(fieldKeys, function(key){  // search recursively for keys not matching the filter
            ...
            else if(input[key] === "" && _.isObject(schema.prototype._schema.paths[key]) &&
                    schema.prototype._schema.paths[key].instance === "ObjectID"){
